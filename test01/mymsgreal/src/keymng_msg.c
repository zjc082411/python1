...
int TeacherEncode_(Teacher *pTeacher /*输入 in*/, ITCAST_ANYBUF **outData)
{
	int			ret = 0;
...
int  TeacherDecode_(unsigned char *indata, int inLen, Teacher **pStruct)
{
	...
	{
		ret = 1;
		printf("func TeacherDecode() err:%d malloc err \n", ret);
		return ret;
	}
	memset(inAnyBuf, 0, sizeof(ITCAST_ANYBUF)); //很重要
	inAnyBuf->next = NULL;
	inAnyBuf->prev = NULL; //废话
